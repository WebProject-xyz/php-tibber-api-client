...
class TodayPricesCommand extends AbstractTibberCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $io->title(sprintf("Today's Tibber Energy Prices (Home: %s, Resolution: %s)", $homeId, $resolution->value));

        $prices = $service->getTodaysPrices($homeId, $resolution);

        if ([] === $prices) {
            $io->warning('No price data available for today.');
